  ...
  <div class="container" v-if="editShow">
    ...
    >
      ...
      <div class="mb-3">
        ...
        <vee-field name="nr" type="number" placeholder="Enter card number" />
        <error-message class="text-red-600" name="nr" />
      </div>
      <!-- PIN -->
      <div class="mb-3">
        <label class="inline-block mb-2">PIN</label>
        <vee-field type="number" name="pin" placeholder="Enter PIN" />
...
export default {
  ...
  methods: {
    async deleteCard() {
      ...
      if (prompt) {
        try {
          await fetch('https://test.boncard.pl/rekrutacja/api/delete.php', {
            method: 'POST',
            mode: 'cors',
            headers: {
    ...
    async saveEditCard(data) {
      ...
      try {
        await fetch('https://test.boncard.pl/rekrutacja/api/update.php', {
          method: 'POST',
          mode: 'cors',
          headers: {
            'Content-Type': 'application/json',
